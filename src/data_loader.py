...
class InsuranceDataLoader:
    ...
    def create_derived_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "IssueDate" in df_feat.columns and "ExpiryDate" in df_feat.columns:
            ...
            df_feat["ExpiryDate"] = pd.to_datetime(
                df_feat["ExpiryDate"], errors="coerce"
            )
            df_feat["PolicyDuration"] = (
                df_feat["ExpiryDate"] - df_feat["IssueDate"]
            ).dt.days

        logger.info(
            f"Created {len(df_feat.columns) - len(df.columns)} derived features"
        )
        return df_feat
